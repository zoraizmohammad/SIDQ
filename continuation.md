...
## Where We Left Off (updated 2026-07-21)

The full SIDQ-T2 codebase is implemented and verified (176 tests passing). The project
now targets **Google Colab Pro (T4 GPU)** for training, because the local host is
CPU-only (no GPU) and runs Amazon Linux 2 (glibc 2.26), where `deepfense`/`fairseq`
cannot install from wheels.

### Compute plan
- **All GPU work runs on Colab Pro** via `notebooks/SIDQ_Colab.ipynb`.
- Budget: **100 compute units, no overage** (Colab drops to free tier at 0 — never
  auto-charges). Use **T4** (not A100). A full 30-epoch run is only a few units because
  the XLS-R frontend is frozen — only the 374K-param AASIST head trains.
- The local host is used for: dataset storage, git, and assembling/validating the final
  submission ZIP. It CAN train on CPU (~1 h/epoch) as a fallback but that's the slow path.

### New in this session (native inference path)
- `src/sidq/models/sidq_xlsr.py` — shared XLS-R+AASIST model (single source of truth for
  train + infer, so they can't drift).
- `scripts/infer_sidq.py` — **real-logit** inference for `train_sidq.py` checkpoints.
  (`run_track2_inference.py` is DeepFense-only; `standalone_inference.py` emits a *proxy*
  score — do NOT submit its output.)
- `scripts/train_sidq.py` — added credit guards: `--val-subset` (per-epoch dev subset),
  `--max-hours` (wall-clock budget stop), `--limit-train`, `--resume`, `--attn auto`.
- `notebooks/SIDQ_Colab.ipynb` — turnkey, credit-optimized Colab notebook (generated by
  `scripts/build_colab_notebook.py`).

## Historical note
Training was originally started on a prior machine but interrupted by disk-full. That
machine's downloaded data/checkpoints/submission ZIP are NOT in this clone (they were
gitignored). The dataset + baseline model have since been re-downloaded locally to
`ArA-DF-2026/` (also gitignored).
